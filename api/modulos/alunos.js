...
const getAlunoPorCurso = (nomeDoCurso) => {
    ...
    alunos.forEach(item => {
        item.curso.forEach(item2 => {
            if (nomeDoCurso.toLowerCase() == item2.sigla.toLowerCase()) {
...
// console.table(getAlunoPorCurso("DS"));
...
//console.table(getAluno("20151001001"));


const getDisciplinas = (matricula) => {
    let disciplinas = [];
    let erro = true;

    alunos.forEach(item => {
        if (matricula == item.matricula) {
            item.curso.forEach(cursoItem => {
                cursoItem.disciplinas.forEach(disciplinaItem => {
                    disciplinas.push(disciplinaItem);
                    erro = false;
                })
            })
        }
    ...
    // informacaoAluno.curso.forEach(item => {
    //     item.disciplinas.forEach(index => {
    //         disciplinas.push(index);
    //         erro = false
    //     });
    // });
...
const getAlunoPorStatus = (statusAluno) => {
...
//console.table(getAlunoPorStatus("finalizado"));

let teste = getAlunoPorCurso('ds');

const getFiltrarAlunosPorStatus = (arrayAlunos, statusAluno) => {
    let status = statusAluno.toLowerCase();
    let listaAlunos = arrayAlunos;
...
getFiltrarAlunosPorStatus(teste ,"finalizado");


const getAlunoPorAnoDeConclusao = (ano) => {
    let anoDeConclusao = ano;
    let erro = true;
    let listaAlunos = [];

    alunos.forEach(item => {
        item.curso.forEach(item2 => {
            if (anoDeConclusao == item2.conclusao) {
                listaAlunos.push(item);
                erro = false;
            }
        });
...
// console.table(getAlunoPorAnoDeConclusao("2021"))


const filtrarAlunosPorAnoDeConclusao = (arrayAlunos, ano) => {
    let anoDeConclusao = ano;
    let listaAlunos = arrayAlunos;
    let filtroArray = [];
    let erro = true;
    ...
    listaAlunos.forEach(item => {
        item.curso.forEach(item2 => {
            if (item2.conclusao == anoDeConclusao) {
                filtroArray.push(item);
                erro = false;
            }
        });
 ...
 // console.table(filtrarAlunosPorAnoDeConclusao("2021"))
...
module.exports = {
    ...
    filtrarAlunosPorAnoDeConclusao
}
